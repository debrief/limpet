...
public class Application implements IApplication, IExecutableExtension
{
  ...
  private Object checkInstanceLocation(Shell shell, Map applicationArguments)
  {
    ...
    if (instanceLoc.isSet())
    {
      ...
      catch (IOException e)
      {
        IDEWorkbenchPlugin.log("Could not obtain lock for workspace location", //$NON-NLS-1$
            e);
        MessageDialog.openError(shell, IDEWorkbenchMessages.InternalError,
            e.getMessage());
      }
      return EXIT_OK;
    }
    ...
    while (true)
    {
      ...
      MessageDialog.openError(shell,
          IDEWorkbenchMessages.IDEApplication_workspaceInUseTitle, NLS.bind(
   ...
   * version check will be performed, and a confirmation box may be displayed on the argument shell
  ...
  private boolean checkValidWorkspace(Shell shell, URL url)
  {
    ...
    String message =
        NLS.bind(IDEWorkbenchMessages.IDEApplication_versionMessage_newerWorkspace,
            url.getFile());
  ...
  private static void writeWorkspaceVersion()
  {
    ...
    finally
    {
      ...
      {
        Activator.logError(Status.ERROR, "IO Failure when writing workspace version", e);
      }
    }
  }

  /**
   * The version file is stored in the metadata area of the workspace. This method returns an URL to
   * the file or null if the directory or file does not exist (and the create parameter is false).
   * 
   * @param create
   *          If the directory and file does not exist this parameter controls whether it will be
   *          created.
