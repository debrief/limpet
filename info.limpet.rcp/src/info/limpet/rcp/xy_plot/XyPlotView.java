        ...
        if (coll.size() >= 1)
        {
          if (coll.size() < maxSize)
          {
            ...
            if (coll.size() == 1)
            {
              ...
              Measurable<Quantity> thisValue =
                  thisQ.getValues().iterator().next();
      ...
      if (!coll.isQuantity())
      {
        if (coll.size() >= 1)
        {
          if (coll.size() < maxSize)
          {
            ...
            if (coll.isTemporal())
            {
              info.limpet.data.impl.samples.TemporalLocation loc =
                  (TemporalLocation) coll;
